...
import MangaMap3D from './components/MangaMap3D'
...
function getNeighborsByTagCommunity(manga, tagIdf, tagIndex, communityData, count = 5) {
  ...
  for (const ct of communityTags) {
    ...
    if (net > 0 && tagIndex[ct.tag_name]) {
      adjustedTags.push({ name: ct.tag_name, rank: Math.min(100, 40 + net * 10), spoiler: false })
    }
  }
...
export default function MapApp({ config, mediaType, mapBasePath }) {
  ...
  useEffect(() => {
    if (isFullscreen && selected) {
      ...
      document.title = `${title} — ${config.label}の類似作品 | Media Map`
      setMetaTag('description', `${title}の類似作品・おすすめ${config.label}をAIが分析。`)
    } else {
      document.title = `${config.label}マップ — Media Map`
      setMetaTag('description', `${config.label}の類似作品を探索できるインタラクティブマップ。`)
    }
  ...
  useEffect(() => {
    ...
    if (!initId || mediaData.length === 0) return
    const manga = mediaData.find(m => String(m.id) === String(initId))
    if (!manga) return
  ...
  useEffect(() => {
    if (!mangaId || mediaData.length === 0) return
    const manga = mediaData.find(m => String(m.id) === mangaId)
    if (manga && (!selected || selected.id !== manga.id)) {
      setSelected(manga)
      setTreeRootManga(manga)
      setShowSimilarMap(true)
  ...
  }, [mangaId, mediaData, selected?.id]) // eslint-disable-line

  useEffect(() => {
    if (isFullscreen && selected) navigate(`/${mediaType}/${selected.id}`, { replace: true })
  ...
  const mapNeighbors = useMemo(() => {
    ...
    return computeNeighborList(mapBaseSelected, filteredData, tagIdf, tagIndex, limit, communityMode, communityData)
  }, [mapBaseSelected, filteredData, tagIdf, tagIndex, neighborOnlyMode, neighborOnlyCount, communityMode, communityData])

  const neighbors = useMemo(() => {
    return computeNeighborList(selected, filteredData, tagIdf, tagIndex, 5, communityMode, communityData)
  }, [selected, filteredData, tagIdf, tagIndex, communityMode, communityData])

  useEffect(() => {
    if (!communityMode || !selected) { setCommunityData(null); return }
    fetch(`/api/manga/${encodeURIComponent(selected.id)}/community-tags`)
      .then(r => r.json())
      .then(setCommunityData)
      .catch(() => setCommunityData(null))
  }, [communityMode, selected?.id]) // eslint-disable-line
  ...
  const displayData = useMemo(() => {
    if (neighborOnlyMode && mapBaseSelected && mapNeighbors.length > 0) {
      ...
      return filteredData.filter(m => ids.has(m.id))
    }
    return filteredData
  }, [neighborOnlyMode, mapBaseSelected, mapNeighbors, filteredData])
  ...
  const computeNeighbors = useCallback((item, count = 8) => {
    ...
    if (result.length < count) {
      for (const m of getNeighbors3D(item, filteredData, count * 3)) {
        if (!found.has(m.id)) { result.push(m); found.add(m.id) }
        if (result.length >= count) break
      }
    }
    return result.slice(0, count)
  }, [filteredData, tagIdf, tagIndex])
  ...
  function handleEnterFullscreen() {
    if (selected) navigate(`/${mediaType}/${selected.id}`)
  }
  function handleExitFullscreen() {
    navigate(mapBasePath || `/${mediaType}`)
  }
  function handleGoToMap(mode) {
    const idParam = selected ? `&id=${selected.id}` : ''
  ...
  function handleSearch(query) {
    ...
    const found = filteredData.find(m =>
      m.title.toLowerCase().includes(lower) ||
      (m.title_ja     && m.title_ja.includes(query)) ||
      (m.title_romaji && m.title_romaji.toLowerCase().includes(lower))
    )
    if (found) {
      // 2D表示中は類似マップのルートを変更、そうでなければ3Dフォーカス
      if (showSimilarMap) handleSimilarMapSearch(found)
      else handleSelect(found)
    }
  }
  ...
  if (isMobile) {
    ...
    return (
      <div style={{ width: '100%', height: '100%', display: 'flex', position: 'relative', background: T.appBg }}>
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          {loading ? (
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20, background: T.appBg }}>
              ...
              <div style={{ fontSize: 14, color: T.textSub, letterSpacing: '0.06em' }}>
                {config.label}の世界を構築中…
              </div>
            </div>
          ) : (
            <MangaMap3D
          ...
          {showSimilarMap && treeRootManga && (
            <SimilarityTreeMap
              ...
              onClose={() => setShowSimilarMap(false)}
          ...
          <div style={{ position: 'absolute', zIndex: 10, top: 12, left: 12, right: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <SearchBar mangaData={filteredData} onSearch={handleSearch} onSelect={item => showSimilarMap ? handleSimilarMapSearch(item) : handleSelect(item)} variant={theme === 'dark' ? 'dark' : 'light'} />
            ...
            {history.length > 1 && (
              <div style={{ display: 'grid', gridTemplateColumns: 'auto minmax(0,1fr)', alignItems: 'center', gap: 6, background: theme === 'dark' ? 'rgba(13,17,23,0.96)' : T.headerBg, border: `1px solid ${T.border}`, borderRadius: 12, padding: '7px 10px', backdropFilter: 'blur(18px)', boxShadow: theme === 'dark' ? '0 2px 12px rgba(0,0,0,0.4)' : '0 2px 12px rgba(0,0,0,0.12)' }}>
                <button onClick={handleBack} style={{ ...mobileBtn, background: T.accentBg, border: `1px solid ${T.accentBorder}`, color: T.accent }}>← 戻る</button>
                <div ref={mobileHistoryRef} className="history-scroll" style={{ overflowX: 'auto', display: 'flex', gap: 5, touchAction: 'pan-x', minWidth: 0 }} onTouchStart={e => e.stopPropagation()} onTouchMove={e => e.stopPropagation()}>
                  {history.slice(0, -1).map((item, i, arr) => {
                    ...
                    return (
                      <button key={`${item.id}-${i}`} onClick={() => handleHistoryNav(item, histIdx + 1)}
                        style={{ background: isLatest ? T.accentBg : T.buttonBg, border: `1px solid ${isLatest ? T.accentBorder : T.border}`, borderRadius: 8, color: isLatest ? T.accent : T.textSub, cursor: 'pointer', padding: '5px 10px', fontSize: 11, whiteSpace: 'nowrap', flexShrink: 0, fontWeight: isLatest ? 600 : 400 }}
                      >{item.title}</button>
                    )
                  })}
                </div>
          ...
          <button onClick={() => setCommunityMode(v => !v)}
            style={{ position: 'absolute', bottom: (!!selected) ? 180 : 130, right: 16, zIndex: 12, transition: 'bottom 0.25s ease', padding: '6px 12px', borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 700, background: communityMode ? T.accentBg : T.buttonBg, border: `1px solid ${communityMode ? T.accentBorder : T.border}`, color: communityMode ? T.accent : T.textSub, backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', gap: 5, boxShadow: '0 2px 8px rgba(15,23,42,0.10)' }}
          >
            <span>{communityMode ? '◉' : '◎'}</span>Community
          </button>

          {/* Mobile popularity */}
          <div style={{ position: 'absolute', bottom: !!selected ? 130 : 80, left: 16, zIndex: 10, transition: 'bottom 0.25s ease', display: 'flex', gap: 2, alignItems: 'center', background: T.buttonBg, border: `1px solid ${T.border}`, borderRadius: 12, padding: '6px 10px', backdropFilter: 'blur(18px)', boxShadow: '0 2px 8px rgba(0,0,0,0.10)' }}>
            {[1,2,3,4,5].map(n => (
              <button key={n} onClick={() => setMinPopularity(n === minPopularity ? 1 : n)} style={{ background: 'none', border: 'none', padding: '2px', cursor: 'pointer', fontSize: 18, color: n >= minPopularity && minPopularity > 1 ? POP_COLORS[n] : n >= minPopularity ? T.textSub : T.starDim, textShadow: n >= minPopularity && minPopularity > 1 ? `0 0 8px ${POP_COLORS[n]}88` : 'none', lineHeight: 1 }}>★</button>
            ))}
          </div>

          {/* Mobile favorites */}
          <button onClick={() => setShowFavoritesMap(v => !v)}
            style={{ position: 'absolute', bottom: !!selected ? 130 : 80, right: 24, zIndex: 10, transition: 'bottom 0.25s ease', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 12, background: showFavoritesMap ? 'linear-gradient(135deg,rgba(244,114,182,0.28),rgba(251,113,133,0.18))' : T.buttonBg, border: `1.5px solid ${showFavoritesMap ? '#f472b6' : T.border}`, backdropFilter: 'blur(18px)', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.10)' }}
          >
            <span style={{ fontSize: 15, color: showFavoritesMap ? '#f472b6' : '#604060' }}>♥</span>
            <span style={{ fontSize: 12, fontWeight: 700, color: showFavoritesMap ? '#f9a8d4' : T.textSub }}>マイマップ</span>
            {favorites.size > 0 && <span style={{ fontSize: 10, fontWeight: 700, color: showFavoritesMap ? '#f472b6' : T.accent, background: T.accentBg, borderRadius: 5, padding: '1px 6px' }}>{favorites.size}</span>}
          </button>

          {!loading && (
            <div style={{ position: 'absolute', bottom: 28, left: '50%', transform: 'translateX(-50%)', background: theme === 'dark' ? 'rgba(13,17,23,0.90)' : 'rgba(250,249,245,0.88)', border: `1px solid ${T.border}`, borderRadius: 10, padding: '8px 16px', fontSize: 11, color: T.textSub, pointerEvents: 'none', display: 'flex', gap: 14, whiteSpace: 'nowrap', backdropFilter: 'blur(14px)', boxShadow: theme === 'dark' ? '0 2px 12px rgba(0,0,0,0.5)' : '0 2px 8px rgba(0,0,0,0.10)' }}>
              {[['1本指', '視点移動'], ['2本指', 'ズーム'], ['タップ', '作品を選択']].map(([key, label]) => (
                <span key={key}><span style={{ color: T.accent, fontWeight: 600 }}>{key}</span><span style={{ marginLeft: 4 }}>{label}</span></span>
              ))}
            </div>
          )}
        </div>
        ...
        {selected && (
          <DetailPanel
            ...
            onClose={() => { setSelected(null); setShowSimilarMap(false) }}
            mapMode={true} onSelect={handleSelect}
            ...
            isMobile={true} neighborOnlyMode={neighborOnlyMode}
            onToggleNeighborMode={() => setNeighborOnlyMode(v => !v)}
            allManga={filteredData} communityMode={communityMode}
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: T.appBg, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <aside style={{
          ...
          <div style={{ width: 210, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 16px', borderTop: `1px solid ${T.border}`, flexShrink: 0 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: T.textMuted, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>注目度フィルター</div>
              <div style={{ display: 'flex', gap: 3, marginBottom: 6 }}>
                {[1,2,3,4,5].map(n => {
                  const active = n >= minPopularity; const color = POP_COLORS[n]
                  return (
                    <button key={n} onClick={() => setMinPopularity(n)} title={POP_LABELS[n]}
                      style={{ background: 'none', border: 'none', padding: '2px 3px', cursor: 'pointer', fontSize: 20, color: active ? color : (theme === 'light' ? '#64748b' : '#cbd5e1'), textShadow: active ? `0 0 10px ${color}88` : (theme === 'light' ? '0 1px 0 rgba(15,23,42,0.18)' : 'none'), transition: 'color 0.15s', lineHeight: 1 }}
                      onMouseEnter={e => { e.currentTarget.style.color = color; e.currentTarget.style.transform = 'scale(1.2)' }}
                      onMouseLeave={e => { e.currentTarget.style.color = active ? color : (theme === 'light' ? '#64748b' : '#cbd5e1'); e.currentTarget.style.transform = 'scale(1)' }}
                    >★</button>
                  )
                })}
              </div>
              <div style={{ fontSize: 12, color: POP_COLORS[minPopularity] || T.textSub, fontWeight: 600 }}>
                {minPopularity === 1 ? 'すべて表示' : `★${minPopularity}以上`}
              </div>
            </div>
        ...
        {selected && (
          <DetailPanel
            manga={selected} neighbors={neighbors} tagIdf={tagIdf}
            onClose={(showSimilarMap || showFavoritesMap) ? () => setSelected(null) : () => { setSelected(null); setShowSimilarMap(false) }}
            mapMode={false} onSelect={handleSelect}
            onOpenSimilarMap={handleOpenSimilarMap} showSimilarMap={showSimilarMap}
            selectedTags={selectedTags} onTagClick={handleToggleTag} onClearTags={() => setSelectedTags([])}
            isFavorite={favorites.has(selected.id)} onToggleFavorite={() => toggleFavorite(selected)}
